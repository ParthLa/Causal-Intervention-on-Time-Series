...
class ErrorWindow:
    ...
    def prune_anoms(self, inverse=False):
        ...
        # i_to_remove=int(i_to_remove)
        # for idx in range(len(i_to_remove)):
        #     i_to_remove[idx]=int(i_to_remove[idx])
        #     print("Hi ", i_to_remove[idx])
        i_to_remove = i_to_remove.astype(np.int64)
        if len(i_to_remove) > 0:
            print(type(i_to_remove))
            print(type(i_to_remove[0]))
            print(i_to_remove)
